...
1. Reducing the size of an image by a scale factor k<1 using the nearest-neighbor technique.
- (Bonus): Implementing image reduction by averaging neighboring pixels.

2. Enlarging an image by a scale factor k=2(creating exactly one gap between pixel pairs) using both zero-order and first-order interpolation.

3. Enlarging an image by an integer scale factor k using zero-order and first-order interpolation.
- (Bonus): Implementing enlargement for real scale factors k≥1 using first-order interpolation.
